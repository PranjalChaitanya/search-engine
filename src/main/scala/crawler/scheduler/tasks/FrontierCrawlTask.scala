...
class FrontierCrawlTask(val engine: ExecutionEngine) extends ScheduledTask {
  override def getTasks(): List[() => Unit] = {
    val dispatchWorkflowExecution = DispatchCrawlPageWorkflowFactory.createDispatchCrawlPageWorkflowExecution(engine)
    executeEntireWorkflow(dispatchWorkflowExecution)

    dispatchWorkflowExecution.context.urlList.map(url =>
      CrawlPageWorkflowFactory.createCrawlPageWorkflowExecutionCallback(url, engine)
    )
  }
}
